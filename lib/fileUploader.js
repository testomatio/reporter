...
const uploadUsingS3 = async (filePath, runId) => {
  ...
  try {
    ...
    s3FileLocation = response?.Location;
    debug('Uploaded location', s3FileLocation);

    if (!s3FileLocation) {
      throw new Error(`Problems getting the S3 artifact's link. Please contact the ${APP_PREFIX} team!`);
    }
...
const uploadUsingS3AsBuffer = async (buffer, fileName, runId) => {
  ...
  try {
    ...
    s3BufferLocation = response?.Location;
