...
const uploadUsingS3 = async (filePath, runId) => {
  ...
  try {
    ...
    // Verification that the file was actually created: 10 attempts of 0.5 second
    const isFileExist = await checkFileExists(filePath, 10, 500);
...
const checkFileExists = async (filePath, attempts = 3, intervalMs = 1000) => {
  let fileFound = false;
  let currentAttempt = 0;

  while (!fileFound && currentAttempt < attempts) {
    try {
