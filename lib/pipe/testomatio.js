...
const { APP_PREFIX, STATUS, AXIOS_TIMEOUT, AXIOS_RETRY_TIMEOUT, REPORTER_REQUEST_RETRIES } = require('../constants');
...
class TestomatioPipe {
  constructor(params, store) {
    ...
    axiosRetry(this.axios, {
      ...
      retryDelay: retryCount =>  AXIOS_RETRY_TIMEOUT, // sum = 15sec
      onRetry: retryCount => {
        ...
        if (retriesCountWithinTime.length > REPORTER_REQUEST_RETRIES.maxAmount) {
          throw new Error(
            `${REPORTER_REQUEST_RETRIES.maxAmount} requests to Testomat.io failed within ${Math.floor(
