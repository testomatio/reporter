...
class TestomatioPipe {
   ...
   * @param {string} opts.type - The type of the request (e.g., "tag").
   * @param {string} opts.id - The ID associated with the request.
   * @param {string} opts.apiKey - The API key for authentication.
   * @returns {Promise<string[]>} - An array containing the retrieved test grepList, or an empty array if no tests are found or the request is disabled.
   * @throws {Error} - Throws an error if there was a problem while making the request.
   */
  async prepareRun(opts) {
    if (!this.isEnabled) return [];

    opts.apiKey = this.apiKey.trim();

    const q = generateFilterRequestParams(opts);
    ...
    try {
      ...
      if (Array.isArray(data?.tests) && data?.tests?.length > 0) {
        ...
        console.log(APP_PREFIX, `⛔  No tests found for your --filter --> ${opts.type}=${opts.id}`);
        return [];
      }
    } 
    catch (err) {
...
function generateFilterRequestParams(params) {
  ...
  if (!validTypes.includes(typeLowerCase)) {
    return null;
  }
