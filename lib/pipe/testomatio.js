...
const { APP_PREFIX, STATUS, AXIOS_TIMEOUT, REPORTER_REQUEST_RETRIES, RunStatus } = require('../constants');
...
class TestomatioPipe {
  constructor(params, store) {
    ...
    axiosRetry(this.axios, {
      ...
      retryCondition: error => {
        if (error.response.status === 429) return false; // Do not retry on rate limit exceeded
        return error.response.status >= 400; // Retry on 4xx and 5xx
      },
  ...
  async #cancelRunInCaseOfTooManyReqFailures() {
    ...
    if (retriesCountWithinTime > REPORTER_REQUEST_RETRIES.maxTotalRetries) {
      await this.finishRun({ status: RunStatus.Failed, parallel: this.parallel });

      const errorMessage = chalk.red(
        `❌ Due to ${retriesCountWithinTime} failed requests within ${REPORTER_REQUEST_RETRIES.withinTimeSeconds}s,\
not all tests were reported. Try again later`,
      );
      throw new Error(`${APP_PREFIX} ${errorMessage}`);
    }
  }
  ...
  addTest(data) {
    ...
    this.#cancelRunInCaseOfTooManyReqFailures();
      ...
      .catch(err => {
        if (err.response) {
          if (err.response.status >= 400) {
            const responseData = err.response.data.trim() || { message: '' };
            console.log(
              APP_PREFIX,
