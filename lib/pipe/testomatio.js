...
class TestomatioPipe {
  ...
  async #cancelRunInCaseOfTooManyReqFailures() {
    ...
    if (retriesCountWithinTime > REPORTER_REQUEST_RETRIES.maxTotalRetries) {
      ...
      const errorMessage = chalk.red(`❌ Due to ${retriesCountWithinTime} failed requests within ${
        REPORTER_REQUEST_RETRIES.withinTimeSeconds
      }s, not all tests were reported. Try again later`);
