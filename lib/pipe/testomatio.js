...
class TestomatioPipe {
  constructor(params, store) {
    ...
    // this.axios = axios.create(); //TODO: remove before push
  ...
  async prepareRun(opts) {
    ...
    const url = generateModeRequestURL(opts); //TODO: as example modeRequestStr({"type":"plan", "id":"my plan1"}, KEY)

    try {
      const resp = await this.axios.get(url);
  
      if(resp.data?.tests?.length > 0) {
...
function generateModeRequestURL(params) {
  const ENDPOINT = '/api/test_grep';
  let { type, id, apiKey } = params;
  type = type.toLowerCase();

  const types = {
    "tag": `type=tag`,
    // "plan": `type=plan`, //TODO: for the future
    // "label": `type=label`,
    // "issue": `type=issue`,
    // "jira": `type=jira`
  }
  ...
  const typeParam = types[type];
  const apiParams = `api_key=${apiKey}`;
  const idParam = `id=${id}`;
  const q = [apiParams, typeParam, idParam].join('&');

  return `${ENDPOINT}?${q}`;
}

//TODO: refactor => maybe all this.axios as controller functions
