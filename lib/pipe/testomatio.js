...
const BATCH_UPLOADING_DISABLED = process.env.TESTOMATIO_DEBUG_DISABLE_BATCH_UPLOADING;
...
class TestomatioPipe {
  ...
  async createRun() {
    debug('Creating run...');
    if (!this.isEnabled) return;
    if (!BATCH_UPLOADING_DISABLED) this.batch.intervalFunction = setInterval(this.#batchUpload, this.batch.intervalTime);
 ...
 reporting for test "${testData.title}" to Testomat is skipped`,
   ...
   */
  #batchUpload = async () => {
    if (BATCH_UPLOADING_DISABLED) return;
  ...
  addTest(data) {
    ...
    if (BATCH_UPLOADING_DISABLED) this.#uploadSingleTest(data);
