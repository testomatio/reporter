...
async function pipesFactory(params, opts) {
  ...
  console.log(
    constants_js_1.APP_PREFIX,
    picocolors_1.default.cyan('Pipes:'),
    picocolors_1.default.cyan(pipesEnabled.map(p => p.toString()).join(', ') || 'No pipes enabled'),
  );
  if (!pipesEnabled.length) {
    console.log(
      constants_js_1.APP_PREFIX,
      picocolors_1.default.dim('If you want to use Testomatio reporter, pass your token as TESTOMATIO env variable'),
    );
  }
  return pipes;
}
