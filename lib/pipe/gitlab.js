...
class GitLabPipe {
  ...
  constructor() {
    ...
    //       console.log(APP_PREFIX, 'GitLab Pipe: ', this.token ? 'TOKEN' : '*no token*', this.ref, this.repo);
    //     }
  ...
  async finishRun2() {
    const url = `https://gitlab.com/api/v4/projects/${this.projectID}/merge_requests/${this.mergeRequestID}/notes?access_token=${this.gitlabPAT}`;
  ...
  `
      ...
      try {
        ...
        console.log(
          APP_PREFIX,
          chalk.yellow('GitLab'),
          `Couldn't create GitLab report ${err}`,
        );
      }
    }
