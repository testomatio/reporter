...
class GitLabPipe {
  ...
  constructor(params, store = {}) {
    ...
    this.store = store;
    this.tests = [];
    this.token = params.GITLAB_PAT || ENV.GITLAB_PAT;
  ...
  `;
    ...
    try {
      const addCommentURL = `https://gitlab.com/api/v4/projects/${this.ENV.CI_PROJECT_ID}/merge_requests/${this.ENV.CI_MERGE_REQUEST_IID}/notes?access_token=${this.token}`;
      console.info(chalk.grey(`Adding comment via url: ${addCommentURL}`));
      const options = {
        json: {
          body,
        },
