...
/**
 * Logger allows to:
 * 1. Intercept logs from user logger (console, pino, tracer, etc).
 * 2. Output logs to console (actually, logger functionality).
 * 3. Varied syntax to fix any user preferences.
 */
class Logger {
  // #intercepted and reassigned immediately when added
  #loggerToIntercept;
  ...
  #parseLastArgToGetTestId(...args) {
    ...
    // replace with next line when node 14 is deprecated
   ...
   * Supports tagged template syntax: step`login success`
   * @param {*} strings
   * @param  {...any} values
   */
  step(strings, ...values) {
  ...
  _log(strings, ...args) {
    ...
    // try to get testId for mocha
    context = global.testTitle ?? null;

    // last argument could contain testId
    const testId = this.#parseLastArgToGetTestId(...args);
    if (testId) {
  ...
  _logWrapper(argsArray, level) {
    ...
    // last argument could contain testId
    const testId = this.#parseLastArgToGetTestId(...argsArray);
    if (testId) {
      // last arg is test id, do not log it
  ...
  assert(...args) {
    ...
    this.dataStorage.putData(logs);
    try {
      this.#originalUserLogger.log(`Assertion result: `, ...args);
    } catch (e) {
      // method could be unexisting, ignore error
    }
  }
  ...
  /**
   * Intercepts user logger messages and attaches it to testomatio report.
   ...
   * @param {{info?: (...args: any[]) => void }} userLogger // TODO: specify other interceptable methods
   */
  intercept(userLogger) {
    ...
    if (_.isEqual(userLogger, this.#originalUserLogger)) {
    ...
    this.#loggerToIntercept = userLogger;

    // override user logger (any, e.g. console) methods to intercept log messages
    for (const method of LOG_METHODS) {
      this.#loggerToIntercept[method] = (...args) => this[method](...args);
    }
  }
...
// TODO: remove functionality when last argument is used to get testId. not used for any of existing frameworks
