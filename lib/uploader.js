...
class S3Uploader {
  ...
  #getS3Config() {
    ...
    const cfg = {
      region: S3_REGION,
      forcePathStyle: this.#guessForcePathStyle(),
      credentials: {
  ...
  #guessForcePathStyle() {
    const { S3_FORCE_PATH_STYLE, S3_ENDPOINT, S3_BUCKET } = this.getConfig();

    if (S3_FORCE_PATH_STYLE !== undefined && S3_FORCE_PATH_STYLE) {
      return !['false', '0'].includes(String(process.env.S3_FORCE_PATH_STYLE || '').toLowerCase());
    }

    // do not use subdomain format if . or capitals present
    if (S3_BUCKET.includes('.')) return true;
    if (S3_BUCKET !== S3_BUCKET.toLowerCase()) return true;
