...
class S3Uploader {
   ...
   * @param {*} Body
   * @param {*} Key
   * @param {{path: string, sizeMB?: number}} file
   * @returns
   */
  async #uploadToS3(Body, Key, file) {
    ...
    try {
      ...
      this.failedUploads.push({ path: file.path, sizeMB: file.sizeMB });
  ...
  async uploadFileByPath(filePath, pathInS3) {
    ...
    if (!this.isEnabled) {
      ...
      this.skippedUploads.push({ file: filePath });
      return;
    }
    ...
    if (TESTOMATIO_ARTIFACT_MAX_SIZE_MB && fileSizeInMb > parseInt(TESTOMATIO_ARTIFACT_MAX_SIZE_MB, 10)) {
      const skippedArtifact = { file: filePath, sizeInMBytes: fileSizeInMb };
    ...
    const link = await this.#uploadToS3(fileStream, Key, { path: filePath, sizeMB: fileSizeInMb });
