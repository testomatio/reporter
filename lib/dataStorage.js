...
Thus need to use file storage. But when tests are run in parallel, the new problem occurs – several files required.
Tried to solve it by using the TEST_WORKER_INDEX env variable (which is part of the filename),
but TEST_WORKER_INDEX var is not available inside the listener (available only inside the test).
Thus cannot match currently running testId from the reporter listener with the actual currently running test.
