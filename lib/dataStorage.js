...
class DataStorage {
  ...
  constructor(dataType, params = {}) {
    ...
    */
    ...
    if (this.isFileStorage) {
      this.dataDirPath = path.join(TESTOMAT_TMP_STORAGE.mainDir, this.dataDir);
   ...
   * @param {*} context is used inside some test runners to define test id
   * @returns
   */
  ...
  _getDataFromGlobalVar(testId) {
    try {
      if (global?.testomatioDataStore[this.dataDir]) {
        const testData = global.testomatioDataStore[this.dataDir][testId];
  ...
  _putDataToGlobalVar(data, testId) {
    ...
    global.testomatioDataStore[this.dataDir] = {};
    global.testomatioDataStore[this.dataDir][testId] = data;
  }
