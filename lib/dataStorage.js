...
class DataStorage {
   ...
   * Stores data to global variable or to file depending on what is applicable for current test runner (running environment).
  ...
  putData(data, context = null) {
    ...
    // explicitly set file storage for mocha (it creates global variable storage too, but it should be ignored)
    if (this.runningEnvironment === 'mocha') this.isFileStorage = true;
  ...
  getData(context) {
    ...
    */
    ...
    // TODO: [fix required] test data becomes 'null' (string) here even if null (as value type) returned
    if (testData === 'null') testData = null;
