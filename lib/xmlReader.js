...
class XmlReader {
  ...
  connectAdapter() {
    if (this.opts.javaTests) return this.adapter = adapterFactory('java', this.opts);
    return this.adapter = adapterFactory(this.stats.language, this.opts);
  }
  ...
  formatTests() {
    this.tests.forEach(t => {
      ...
      t.title = t.title
        // Insert a space before all caps, except when preceded by a letter
        .replace(/([a-z])([A-Z])/g, '$1 $2')
        // Convert the whole word to uppercase if it's all uppercase
