...
const { fetchFilesFromStackTrace, fetchIdFromOutput, fetchSourceCode, fetchSourceCodeFromStackTrace, fetchIdFromCode } = require('./util');
...
function reduceTestCases(prev, item) {
  ...
  testCases.filter(t => !!t).forEach(testCaseItem => {
    ...
    testId = fetchIdFromOutput(stack);
