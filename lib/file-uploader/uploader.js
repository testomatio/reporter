...
class Uploader {
     ...
     * @returns {string|Buffer} The file extension corresponding to the base64-encoded buffer if the buffer exists, or the buffer itself if it is falsy.
     */
    get fileExtBase64() {
     ...
     * @returns {string} The file extension corresponding to the input string, or an empty string if no matching extension is found.
     */
    fileExt64(str) {
    ...
    unsuccessfullBufferUploadMsg() {
        if (this.bufferKey) {
            ...
            console.log(this.prefix, chalk.bold.red(`Failed uploading '${this.bufferKey}'. Please check you STORAGE credentials!`));
            console.log(this.prefix, '---------------');
        }
        if (this.fileKey) {
            console.log(this.prefix, '---------------');
            console.log(this.prefix, chalk.bold.red(`Failed uploading '${this.fileKey}'. Please check you STORAGE credentials!`));
            console.log(this.prefix, '---------------');
        }
    }
    /**
     * Retrieves the contents of a file based on its filepath.
     * @returns {Buffer|undefined} The contents of the file as a Buffer if the file exists, or undefined if the file does not exist.
     */
    get fileByPath() {
    ...
    /**
     * Logs a message to disable artifact uploads by setting the TESTOMATIO_DISABLE_ARTIFACTS environment variable.
     */   
    ...
    /**
     * Logs a warning message related to access to storage for uploading artifacts.
     */
     ...
     * @returns {Object} An object containing the retrieved environment variables, where the keys are the provided environment variable keys and the values are the corresponding values from the process environment.
     */
    ...
    static isAzureArtifactsEnabled() {
        const enabled = !!(process.env.AZURE_ACCOUNT_KEY && process.env.AZURE_CONTAINER_NAME && !process.env.TESTOMATIO_DISABLE_ARTIFACTS);
