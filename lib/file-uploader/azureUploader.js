...
class AzureUploader extends Uploader {
     ...
     * If the file or buffer is successfully uploaded, it returns the URL of the 
     * uploaded file.
     * If the file or buffer is marked as private (isPrivate = true), it returns
     * a readonly link with a shared access signature (SAS) token.
     *
     ...
     * uploaded file or a readonly link with a SAS token.
     * @throws {Error} If the required parameters (connectionString, containerName, file/buffer)
     * are not provided.
     */
     ...
     *
     * @returns {Promise<string|undefined>} A promise that resolves to the URL 
     * of the uploaded file or a readonly link with a SAS token.
     * @throws {Error} If the required parameters (connectionString, 
     * containerName, file/buffer) are not provided.
     */

    async uploadFileToBucket() {
        const isPrivate = !!this.azureConfig.TESTOMATIO_PRIVATE_ARTIFACTS;
