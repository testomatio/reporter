...
function setS3Credentials(artifacts) {
    ...
    if (artifacts.ACCESS_KEY_ID) config.S3_ACCESS_KEY_ID = artifacts.ACCESS_KEY_ID;
    if (artifacts.SECRET_ACCESS_KEY) config.S3_SECRET_ACCESS_KEY = artifacts.SECRET_ACCESS_KEY;
    if (artifacts.REGION) config.S3_REGION = artifacts.REGION;
    if (artifacts.BUCKET) config.S3_BUCKET = artifacts.BUCKET;
    if (artifacts.ENDPOINT) config.S3_ENDPOINT = artifacts.ENDPOINT;
    if (artifacts.presign) config.TESTOMATIO_PRIVATE_ARTIFACTS = '1';
    resetConfig();
}
/**
