...
class Logger {
  ...
  static getInstance() {
    if (!this.#instance) {
      this.#instance = new Logger({ isFileStorage: true });
    }
  ...
  // TODO: isFileStorage should be moved our of constructor and should be set by setStorageType method
  /**
   *
   * @param {*} isFileStorage - defines if logs should be stored in file or in global variable
   */
  constructor({ isFileStorage }) {
    this.isFileStorage = isFileStorage;

    this.#dataStorage = new DataStorage('log', { isFileStorage: this.isFileStorage });
...
// module.exports.logger = new Logger({ isFileStorage: true });
