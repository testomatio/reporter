...
class DataStorage {
   ...
   * @returns array of data (any type) or string)
   */
  getData(context) {
    this.#refreshStorageType();
    ...
    if (global?.testomatioDataStore) {
      ...
      if (testDataFromGlobalVar && ['cucumber', 'codeceptjs'].includes(this.runningEnvironment)) {
        ...
        if (this.dataType === 'log') return testDataFromGlobalVar.join('\n');
    ...
    */
    ...
    if (!testDataFromFile) debug(`No "${this.dataType}" data for test id ${testId} in both file and global variable`);
  ...
  #getDataFromFile(testId) {
    try {
      ...
      if (fs.existsSync(filepath)) {
        ...
        if (testData) debug(`"${this.dataType}" data for test id ${testId}:`, testData);
        return testData || '';
      }
