...
class DataStorage {
  ...
  putData(data, context = null) {
    ...
    context = context || global.testomatioTestTitle || jestHelpers.getIdOfCurrentlyRunningTest();
    if (!context) {
   ...
   * @returns {any []} of data (any type), null (if no data found for context) or string (if data type is log)
   */
  getData(context) {
    if (!context) {
      debug(`Trying to get "${this.dataType}" data without context`);
      return null;
    }
    ...
    debug(`No "${this.dataType}" data for context "${context}" in both file and global variable`);
  ...
  #getDataFromGlobalVar(context) {
    try {
      if (global?.testomatioDataStore[this.dataType]) {
        ...
        if (testData) debug(`"${this.dataType}" data for constext "${context}":`, testData.join(', '));
        return testData || [];
      }
  ...
  #getDataFromFile(context) {
    try {
      ...
      if (fs.existsSync(filepath)) {
        ...
        if (testDataAsText) debug(`"${this.dataType}" data for context "${context}":`, testDataAsText);
