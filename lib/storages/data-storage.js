...
const { fileSystem, jestHelpers } = require('../util');
...
class DataStorage {
  ...
  #putDataToGlobalVar(data, testId) {
    ...
    global.testomatioDataStore?.[this.dataType][testId] || (global.testomatioDataStore[this.dataType][testId] = []);
...
/* does not work for Playwright
    if (!context && !global.testTitle) {
      // set test title for playwright (does not work from playwright adapter)
      try {
...
*/
// Playwright storage works fine only when running tests from a single file; otherwise it is not possible to define test id/title, it is overwritten
// TODO: the only implementation for PW for now is to pass testInfo inside test (and process this testInfo inside "put" function)
