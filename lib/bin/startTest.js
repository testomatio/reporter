...
program
  ...
  .option("--filter <filter>", "Additional execution filter") //TODO: mode change to.... + Additionals options to pipe or "--execution-mode"
  .action(async (opts) => {
    let { command, launch, finish, filter } = opts;
    ...
    if(filter) {
      ...
      const id = params[0]; //TODO: in the feature = mode.split("=")[1].split("|") - if more than 1 value;

      try {
        const tests = await client.prepareRun({ type, id });

        if(tests?.length === 0) {
          return;
        }

        console.log(APP_PREFIX, `✅ We found ${tests.length} test/-s!`);
