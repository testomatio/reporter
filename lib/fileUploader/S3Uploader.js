...
const memoize = require('lodash.memoize'); //TODO: need to use it in this place
...
class S3Uploader extends Uploader {
    ...
    constructor(opts) {
        super(opts);
        this.keys = UPLOAD_S3_KEYS;

        if (this.keys.length > 1) {
            this.s3Config = Uploader.envPreparation(this.keys);
    ...
    get isS3ArtifactsEnabled() {
        if (this.enabled !== undefined) return this.enabled;

        this.enabled = !!(this.s3Config.S3_BUCKET && !this.s3Config.TESTOMATIO_DISABLE_ARTIFACTS);
    ...
    s3WrongAccesseWarning() {
        //TODO: think how to update using this var
        const {
    ...
    // get getUploadFileByPath() {
    //     return memoize(this.uploadFileByPath);
    //     //TODO: think about memoize
    // }

    async uploadFileByPath() {
        // check that artifacts is enabled
        const enabled = this.isS3ArtifactsEnabled;

        try {
            if (enabled) {
              return this.s3Uploading();
            }
        } catch (e) {
            this.uploadingBucketError("S3", e);
        }
    }
    ...
    async uploadFileAsBuffer() {
        ...
        try {
            if (enabled) {
              return this.s3AsBufferUploading();
            }
          } catch (e) {
            this.uploadingBucketError("S3", e);
          }
