...
class JestReporter {
  ...
  onTestStart(testFile) {
    ...
    // storages.setContext(testFile.path);
    logger.setContext(testFile.path);
  }

  // start of the test (including beforeEach)
  onTestCaseStart(test, testCase) {
    debug('Start running test:', testCase.fullName);
    // storages.setContext(testCase.fullName);
    logger.setContext(testCase.fullName);
  }
  ...
  onTestResult(test, testResult) {
    ...
    for (const result of testResults) {
      ...
      const logs = getTestLogs(testResult);
      // const artifacts = storages.artifacts.get(testResult.fullName);
      // const keyValues = storages.keyValues.get(testResult.fullName);
...
function getTestLogs(testResult) {
  // const suiteLogsArr = storages.logger.getLogs(testResult.testFilePath);
  const suiteLogsArr = logger.getLogs(testResult.testFilePath);

  const suiteLogs = suiteLogsArr ? suiteLogsArr.join('\n').trim() : '';
  // const testLogsArr = storages.logger.getLogs(testResult.fullName);
  const testLogsArr = logger.getLogs(testResult.fullName);
