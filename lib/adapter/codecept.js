...
function CodeceptReporter(config) {
  ...
  event.dispatcher.on(event.suite.before, suite => {
    ...
    storages.setContext(suite.fullTitle());
  });

  event.dispatcher.on(event.suite.after, () => {
    storages.setContext(null);
  });
  ...
  event.dispatcher.on(event.test.before, test => {
    ...
    storages.setContext(test.fullTitle());
  });
  ...
  event.dispatcher.on(event.test.passed, test => {
    ...
    const manuallyAttachedArtifacts = storages.artifacts.get(test.fullTitle());
    const keyValues = storages.keyValues.get(test.fullTitle());
    storages.setContext(null);
...
function getTestLogs(test) {
  const suiteLogsArr = storages.logger.getLogs(test.parent.fullTitle());
  const suiteLogs = suiteLogsArr ? suiteLogsArr.join('\n').trim() : '';
  const testLogsArr = storages.logger.getLogs(test.fullTitle());
