...
function CodeceptReporter(config) {
  ...
  event.dispatcher.on(event.suite.before, suite => {
    ...
    artifactStorage.setContext(suite.fullTitle());
  });

  event.dispatcher.on(event.suite.after, () => {
    logger.setContext(null);
    artifactStorage.setContext(null);
  });
  ...
  event.dispatcher.on(event.test.before, test => {
    ...
    artifactStorage.setContext(test.fullTitle());
  });
  ...
  event.dispatcher.on(event.test.passed, test => {
    ...
    const manuallyAttachedArtifacts = artifactStorage.get(test.fullTitle());
    logger.setContext(null);
    artifactStorage.setContext(null);
