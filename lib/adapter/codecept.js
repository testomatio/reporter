...
function CodeceptReporter(config) {
  ...
  event.dispatcher.on(event.test.before, test => {
    ...
    logger.setContext('BEFORE_TEST_' + test.fullTitle());
  });
...
function getTestLogs(test) {
  const suiteLogs = logger.getLogs(test.parent.fullTitle()).join('\n').trim();
  const beforeTestLogs = logger.getLogs('BEFORE_TEST_' + test.fullTitle()).join('\n').trim();
  const testLogs = logger.getLogs(test.fullTitle()).join('\n').trim();
  let logs = '';
  if (suiteLogs) {
    logs += `${chalk.bold('\tSuite logs:')}\n${suiteLogs}`;
  }
  if (beforeTestLogs) {
