...
const logger = new Logger({ isFileStorage: false });
// example
// logger.setStorageType('global');
...
function CodeceptReporter(config) {
  ...
  event.dispatcher.on(event.test.before, test => {
    ...
    logger.setContext('BEFORE_TEST' + test.fullTitle());
  });
...
function getLogs(test) {
  const suiteLogs = logger.getLogs(test.parent.fullTitle());
  const beforeTestLogs = logger.getLogs('BEFORE_TEST' + test.fullTitle());
  const testLogs = logger.getLogs(test.fullTitle());
  const logs = `${chalk.bold('\tSuite logs:')}\n${suiteLogs || ''}
    ${chalk.bold('Before test logs:')}\n${beforeTestLogs || ''}
    ${chalk.bold('Test logs:')}\n${testLogs || ''}`.trim();
