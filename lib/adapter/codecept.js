...
function CodeceptReporter(config) {
  ...
  event.dispatcher.on(event.test.passed, test => {
    global.testomatioDataStore.logsBeforeAll = null;
    ...
    client.addTestRun(STATUS.PASSED, {
      ...
      // stack: logger.getLogs(getIdFromTestTitle(title)),
      test_id: testId,
    });
    // output.stop();
  });
    ...
    for (let i = 0; i < Math.max(currentMetaStep.length, metaSteps.length); i++) {
      if (currentMetaStep[i] !== metaSteps[i]) {
        ...
        if (metaSteps[i].isBDD()) {
          ...
          global.testomatioDataStore.steps.push(repeat(stepShift) + chalk.bold(metaSteps[i].toString()) + metaSteps[i].comment);
        } else {
