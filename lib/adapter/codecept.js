...
module.exports = (config) => {
  ...
  event.dispatcher.on(event.test.failed, (test) => {
    ...
    recorder.add('Adding TestRun status', () => client.addTestRun(testId, TRConstants.FAILED, '', title));
  });
  ...
  event.dispatcher.on(event.test.passed, (test) => {
    ...
    const testId = parseTest(tags);
    recorder.add('Adding TestRun status', () => client.addTestRun(testId, TRConstants.PASSED, '', title));
  });
};
