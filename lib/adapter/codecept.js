...
function CodeceptReporter(config) {
  ...
  //   console.log(' - - - - - -  - - - ');
  //   console.log(`📗 Suite:`);
  //   console.log(Object.keys(suite.title));
  //   console.log(' - - - - - -  - - - ');
  // });
  ...
  event.dispatcher.on(event.test.before, test => {
    ...
    const context = stringToFilename(test.fullTitle());
  ...
  event.dispatcher.on(event.test.passed, test => {
    ...
    const context = stringToFilename(test.fullTitle());
    const logs = logger.getLogs(context);
...
function stringToFilename(inputString) {
  ...
  return inputString.replace(validFilenameRegex, '_').replace(/_{2,}/g, '_');
}
