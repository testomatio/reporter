...
class PlaywrightReporter {
  ...
  onTestEnd(test, result) {
    ...
    if (isTestShouldBeExculedFromReport(test)) return;
...
function isTestShouldBeExculedFromReport(test) {
  const fileName = path.basename(test.location?.file || '');
  const excludingRegexStr = process.env.TESTOMATIO_PLAYWRIGHT_EXCLUDE_TESTS_FROM_REPORT_REGEX;
  if (excludingRegexStr) {
    const regex = new RegExp(excludingRegexStr);
    if (regex.test(fileName)) {
      debug(`Excluding test '${test.title}' <${fileName}> from reporting`);
      return true;
    }
  }
  return false;
