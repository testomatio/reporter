...
const { isArtifactsEnabled } = require('../file-uploader/uploadManager');
...
class TestomatioReporter {
  ...
  onTestEnd(test, result) {
    ...
    this.totalArtifacts += result.attachments.length;
  ...
  async onEnd(result) {
    ...
    if (isArtifactsEnabled() && this.totalArtifacts === 0) console.log(APP_PREFIX, `🎞️  No files to upload!`);

    if (isArtifactsEnabled() && this.totalArtifacts > 0) {
