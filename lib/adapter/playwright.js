...
class PlaywrightReporter {
  ...
  onTestBegin(testInfo) {
    const fullTestTitle = getTestFullTitle(testInfo);
  ...
  onTestEnd(test, result) {
    ...
    const fullTestTitle = getTestFullTitle(test);
    let logs = '';
 ...
 * @returns 
 */
function getTestFullTitle(test) {
  return `${test.parent.title}_${test.title}`
}
...
function initPlaywrightForStorage() {
  try {
    ...
    test.beforeEach(async ({}, testInfo) => {
      const relateveFilepath = path.relative(process.cwd(), testInfo.file);
      global.testomatioTestTitle = `${relateveFilepath}_${testInfo.title}`;
    });
  } catch (e) {
    // ignore
  }
