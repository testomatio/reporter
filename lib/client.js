...
const TESTOMATIO_RUNGROUP_TITLE = process.env.TESTOMATIO_RUNGROUP_TITLE;
const TESTOMATIO_ENV = process.env.TESTOMATIO_ENV;
...
class TestomatClient {
   ...
   * @returns {Promise}
   */
  addTestRun(testId, status, testData = {}) {
    let {
      message = '',
      error = '',
      title = '',
    ...
    if (error) {
      try {
        const record = createCallsiteRecord({ forError: error,
          processFrameFn: frame => frame.getFileName().indexOf('node_modules') < 0
         });
        stack = record.renderSync({ stackFilter: frame => {
          return frame.getFileName().indexOf(sep) > -1 && frame.getFileName().indexOf('node_modules') < 0
        }});
