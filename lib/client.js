...
const upload = require('./file-uploader/uploadManager');
...
class Client {
  ...
  async addTestRun(status, testData, storeArtifacts = []) {
    ...
    for (const file of files) {
      uploadedFiles.push(upload.fileUploadByPath(file, this.uuid));
    }

    for (const [idx, buffer] of filesBuffers.entries()) {
      const fileName = `${idx + 1}-${title.replace(/\s+/g, '-')}`;
      uploadedFiles.push(upload.fileUploadByBuffer(buffer, fileName, this.uuid));
    }
    ...
    this.totalUploaded += artifacts.filter(n => n !== undefined).length;
