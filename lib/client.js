    ...
    this.queue = this.queue
      .then(async () => {
        ...
        if (this.runUrl && this.proceed) {
          console.log(APP_PREFIX, `📊 ${chalk.yellow.bold('Run was not finished')} because of $TESTOMATIO_PROCEED. Report URL: ${chalk.magenta(this.runUrl)}`);
          console.log(APP_PREFIX, `🛬 Run this command to finish the run:   TESTOMATIO_RUN=${this.runId} npx start-test-run --finish`);
        }
      })
      .catch(err => {
