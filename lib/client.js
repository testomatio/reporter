...
class Client {
  ...
  async addTestRun(status, testData) {
    ...
    if (await isTestShouldBeExculedFromReport(testData)) return [];
 ...
 * @returns boolean
 */
async function isTestShouldBeExculedFromReport(testData) {
  ...
  // as scanning files is async operation, just save the result in variable to avoid multiple scans
  if (!listOfTestFilesToExcludeFromReport) {
    // list of files with relative paths
    listOfTestFilesToExcludeFromReport = await glob(excludeParretnsList, { ignore: '**/node_modules/**' });
  ...
  // no files found matching the excusion pattern
