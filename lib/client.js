    ...
    updateRunStatus(status, isParallel = false) {
        ...
        this.queue = this.queue
            ...
            if (this.uploader.totalUploadsCount && this.uploader.isEnabled) {
                console.log(constants_js_1.APP_PREFIX, `🗄️ ${this.uploader.totalUploadsCount} artifacts ${process.env.TESTOMATIO_PRIVATE_ARTIFACTS ? 'privately' : picocolors_1.default.bold('publicly')} uploaded to S3 bucket`);
                if (this.uploader.failedUploadsCount) {
                    console.log(constants_js_1.APP_PREFIX, picocolors_1.default.gray('[CLIENT]'), `${this.uploader.failedUploadsCount} artifacts failed to upload`);
                }
                if (this.uploader.isEnabled && this.uploader.skippedUploadsCount) {
                    console.log(constants_js_1.APP_PREFIX, `${picocolors_1.default.bold(this.uploader.skippedUploadsCount)} artifacts skipped to upload`);
                }
