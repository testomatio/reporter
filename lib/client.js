    ...
    this.queue = this.queue
      ...
      .then(() => {
        ...
        if (this.uploader.failedUploads.length) {
          console.group(APP_PREFIX, `🗄️ ${this.uploader.failedUploads.length} artifacts failed to upload`);

          for (const { path, sizeMB } of this.uploader.failedUploads) {
            console.log(APP_PREFIX, path, chalk.gray(`[${sizeMB} MB]`));
          }
          console.groupEnd();
        }
        ...
        if (this.uploader.isEnabled && this.uploader.skippedUploads.length) {
          console.group(
            APP_PREFIX,
            `🗄️ ${chalk.bold(this.uploader.skippedUploads.length)} artifacts uploading skipped (due to large size)`,
          );

          for (const { path, sizeMB } of this.uploader.skippedUploads) {
            console.log(APP_PREFIX, path, chalk.gray(`[${sizeMB} MB]`));
          }
          console.groupEnd();
        }
        ...
        if (this.uploader.skippedUploads.length || this.uploader.failedUploads.length) {
          const apiKey = process.env.TESTOMATIO || '<your_api_key>';
          const command = `TESTOMATIO=${apiKey} TESTOMATIO_RUN=${this.runId} npx @testomatio/reporter upload-artifacts`;
          console.log(
            APP_PREFIX,
...
// artifacts are not uploaded in addTestRun method, because "files" prop is not passed to the method
// inside client.updateRunStatus method there is analyzing of uploaded artifacts
// BUT: artifacts are uploaded in playwright adapter onEnd method after analyzing
// thus, artifact uploading stat is useless
