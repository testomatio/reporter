...
class TestomatClient {
   ...
   * @returns {Promise}
   */
  async addTestRun(testId, status, testData = {}, adapterArtifacts = undefined) {
    // all pipes disabled, skipping
    if (!this.pipes.filter(p => p.isEnabled).length) return;
    ...
    if (Array.isArray(adapterArtifacts) && adapterArtifacts.length) {
      files.push(...adapterArtifacts);
      adapterArtifacts = [];
    }
