...
class TestomatClient {
   ...
   * @returns {Promise}
   */
  async addTestRun(testId, status, testData = {}, artifacts = undefined) {
    // all pipes disabled, skipping
    if (!this.pipes.filter(p => p.isEnabled).length) return;
    ...
    //TODO: нужно еще проверить что бы не было кейса когда global.testomatioArtifacts и artifacts вместе отработали
    if (Array.isArray(artifacts) && artifacts.length) {
      files.push(...artifacts);
      artifacts = [];
    }

    if (Array.isArray(global.testomatioArtifacts) && global.testomatioArtifacts.length) { //TODO: need to test with new condition
