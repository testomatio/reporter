...
import {uploadFileAsBuffer, uploadFileByPath, isArtifactsEnabled} from './fileUploader.js';
...
class Client {
  ...
  async addTestRun(status, testData) {
    ...
    for (const f of files) {
      uploadedFiles.push(uploadFileByPath(f, this.uuid));
    }

    for (const [idx, buffer] of filesBuffers.entries()) {
      const fileName = `${idx + 1}-${title.replace(/\s+/g, '-')}`;
      uploadedFiles.push(uploadFileAsBuffer(buffer, fileName, this.uuid));
    }
