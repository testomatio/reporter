    ...
    this.queue = this.queue
      ...
      .then(() => {
        ...
        if (this.uploader.failedUploads.length) {
          console.log(APP_PREFIX, `🗄️ ${this.uploader.failedUploads.length} artifacts 🔴${chalk.bold('failed')} to upload`);
          const failedUploads = this.uploader.failedUploads.map(({ path, size }) => ({
            relativePath: path.replace(process.cwd(), ''),
            sizePretty: prettyBytes(size, { round: 0 }).toString(), // Format size with "MB" suffix
          }));

          const pathPadding = Math.max(...failedUploads.map(upload => upload.relativePath.length)) + 1;
          failedUploads.forEach(upload => {
            console.log(
              `  ${chalk.gray('|')} 🔴 ${upload.relativePath.padEnd(pathPadding)} ${chalk.gray(`| ${upload.sizePretty.padStart(7)} |`)}`,
            );
          });
        }
        ...
        if (this.uploader.isEnabled && this.uploader.skippedUploads.length) {
          console.log(
            APP_PREFIX,
            `🗄️ ${chalk.bold(this.uploader.skippedUploads.length)} artifacts uploading 🟡${chalk.bold('skipped')} (due to large size)`,
          );

          const skippedUploads = this.uploader.skippedUploads.map(({ path, size }) => ({
            relativePath: path.replace(process.cwd(), ''),
            sizePretty: size === null ? 'unknown' : prettyBytes(size, { round: 0 }).toString(),
          }));

          const pathPadding = Math.max(...skippedUploads.map(upload => upload.relativePath.length)) + 1;

          // Log each row without a header
