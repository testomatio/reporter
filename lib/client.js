...
class Client {
  ...
  prepareRun(opts) {
    console.log("********************** prepareRun = Reporter #167 Additional Plan Name/ID ********************")
    ...
    this.executionList = this.executionList
      .then(() => Promise.all(this.pipes.map(async p => {
        try {
          const result = await p.prepareRun(opts);
          return result;
        }
        catch (err) {
      ...
      .then(filteredResults => {
        if (filteredResults[0]?.length === 0) { //TODO: some BUGs may occur at this line => [0].
          return undefined;
        }
    ...
    debug('Execution tests list', this.executionList);
