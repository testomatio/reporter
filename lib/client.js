...
class Client {
   ...
   * Each pipe in the client is checked for enablement, and if all pipes are disabled, the function returns a resolved Promise.
   ...
   *
   * @param {Object} opts - The options for preparing the test execution list.
   * @returns {Promise<any>} - A Promise that resolves to an array containing the prepared execution list,
  ...
  async prepareRun(opts) {
    ...
    try {
      const results = await Promise.all(this.pipes.map(async p => {
        try {
          return await p.prepareRun(opts);
        } catch (err) {
      ...
      debug('Execution tests list', filteredResults);
