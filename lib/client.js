...
class TestomatClient {
  ...
  createRun() {
    ...
    this.queue = this.queue.then(() => axios.post(`${TESTOMAT_URL.trim()}/api/reporter`, { api_key: this.apiKey.trim() })
      .then(resp => {
  ...
  updateRunStatus(status) {
    this.queue = this.queue.finally(() => {
      if (this.runId) {
        return axios.put(`${TESTOMAT_URL}/api/reporter/${this.runId}`, { api_key: this.apiKey, status });
      }
    }).catch(err => {
