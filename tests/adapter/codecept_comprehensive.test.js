...
describe('CodeceptJS Comprehensive Adapter Tests', function () {
  ...
  describe('Comprehensive Test Scenarios', () => {
    ...
    it('should handle data-driven tests correctly', async () => {
      ...
      const dataTests = testEntries.filter(
        entry => entry.testId.title.includes('data examples') || entry.testId.title.includes('multiple data sets'),
      );
    ...
    it('should handle async tests properly', async () => {
      ...
      const asyncTest = testEntries.find(entry => entry.testId.title.includes('async operation'));
  ...
  describe('Hook Execution Tests', () => {
    it('should execute BeforeSuite and AfterSuite hooks', async () => {
      ...
      const hookTests = testEntries.filter(entry => entry.testId.suite_title === 'Hooks Test Suite');
    ...
    it('should handle failing hooks gracefully', async () => {
      ...
      const failingHookTests = testEntries.filter(entry => entry.testId.suite_title === 'Failing Hooks Test Suite');
  ...
  describe('Error Handling', () => {
    it('should handle tests that throw errors', async () => {
      ...
      const errorTest = testEntries.find(entry => entry.testId.title.includes('throws error'));
    ...
    it('should handle various expect assertions', async () => {
      ...
      const assertionTest = testEntries.find(entry => entry.testId.title.includes('various assertions'));
    ...
    it('should handle string assertion tests', async () => {
      ...
      const stringTest = testEntries.find(entry => entry.testId.title.includes('string assertions'));
  ...
  describe('Reporter Integration', () => {
    it('should capture test steps and execution flow', async () => {
      ...
      const multiStepTest = testEntries.find(entry => entry.testId.title.includes('multiple steps'));
    ...
    it('should handle feature tags properly', async () => {
      ...
      const comprehensiveTests = testEntries.filter(entry => entry.testId.suite_title === 'Comprehensive Test Suite');
    ...
    it('should maintain run consistency across different test types', async () => {
      ...
      const runEvents = debugData.filter(entry => entry.action === 'startRun' || entry.action === 'finishRun');
  ...
  describe('Parallel Workers Support', () => {
    ...
    it('should handle worker failures gracefully', async function () {
      ...
      const intentionalFailures = testEntries.filter(
        entry =>
