...
`;
  ...
  it('should define the CORRECT expected behavior for AfterSuite failures', async () => {
    ...
    const expectedPassedTests = testEntries.filter(entry => entry.testId.title.includes('passing test'));
...
`;
  ...
  after(() => {
    // Clean up test files
    const testFiles = ['aftersuite_failure_test.js', 'beforesuite_failure_test.js'];
