...
describe('ReplayService', () => {
  ...
  describe('parseDebugFile', () => {
    it('should throw error if file does not exist', () => {
      ...
      expect(() => replayService.parseDebugFile(nonExistentFile)).to.throw('Debug file not found');
    });

    it('should throw error if file is empty', () => {
      fs.writeFileSync(debugFile, '');
      expect(() => replayService.parseDebugFile(debugFile)).to.throw('Debug file is empty');
    });
  ...
  describe('replay', () => {
    ...
    it('should use first found run_id from batch entries when no runCreated entry exists', async () => {
      ...
      const debugData = [
        { action: 'createRun', params: { title: 'Test Run' } },
        { action: 'addTestsBatch', runId: firstRunId, tests: [{ id: 'test1', status: 'passed' }] },
    ...
    it('should include run_id in dry run results', async () => {
      ...
      const debugData = [
        { action: 'createRun', params: { title: 'Test Run' } },
        { action: 'addTestsBatch', runId: existingRunId, tests: [{ id: 'test1', status: 'passed' }] },
      ];
  ...
  describe('integration scenarios', () => {
    ...
    it('should handle mixed test data formats', async () => {
      const debugData = [
        ...
        { action: 'addTestsBatch', tests: [{ id: 'batch-test-3', status: 'skipped' }] },
